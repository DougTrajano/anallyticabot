...
class IntentsDiscovery:
    ...
    def search_n_clusters(self, data=None, min_n_clusters=2, max_n_clusters=100, step_n_clusters=1, early_stopping=4):
        ...
        for i in range(min_n_clusters, max_n_clusters, step_n_clusters):
            if in_search:
                ...
                logging.warning("search n_clusters: {}".format(session_data))

                self.search_data.append(session_data)

        self.n_clusters = self.search_data[-2]["n_clusters"]

        logging.warning("best n_clusters: {}".format(self.n_clusters))
    ...
    def text_processing(self, stopwords=False, stopwords_file=None, inplace=True):
        if stopwords:
            if isinstance(stopwords_file, str):
                self._stopwords = load_stopwords(stopwords_file)
            else:
                self._stopwords = nltk.corpus.stopwords.words('portuguese')
